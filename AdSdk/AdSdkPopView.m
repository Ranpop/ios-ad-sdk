...
static NSTimeInterval const rFadeInAnimationDuration = 0.7;
static NSTimeInterval const rTransformPart1AnimationDuration = 0.3;
static NSTimeInterval const rTransformPart2AnimationDuration = 0.4;
static CGFloat const rDefaultCloseButtonPadding = 17.0f;
...
@interface AdSdkPopView () <AdSdkPopViewDelegate>
...
@end
@implementation AdSdkPopView

+(AdSdkPopView *)sharedInstance {
    static AdSdkPopView *sharedClient = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        sharedClient = [[AdSdkPopView alloc]init];
    });
    return sharedClient;
}
...
- (void)setCloseButtonType:(ButtonPositionType)closeButtonType {
    ...
    if (closeButtonType == ButtonPositionTypeNone) {
        ...
        if(closeButtonType == ButtonPositionTypeRight){
            closeFrame.origin.x = round(CGRectGetWidth(self.containerView.frame) - rDefaultCloseButtonPadding - CGRectGetWidth(closeFrame)/2);
        } else {
...
- (void)showWithPresentView:(UIView *)presentView animated:(BOOL)animated {
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        if(animated){
            ...
            [UIView animateWithDuration:rFadeInAnimationDuration animations:^{
            ...
            [UIView animateWithDuration:rTransformPart1AnimationDuration animations:^{
                ...
                [UIView animateWithDuration:.2 animations:^{
                    ...
                    [UIView animateWithDuration:rTransformPart2AnimationDuration delay:0 options:UIViewAnimationOptionCurveEaseOut animations:^{
                        containerView.alpha = 1;
...
- (void)hideAnimated:(BOOL)animated withCompletionBlock:(void(^)())completion {
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        [UIView animateWithDuration:rTransformPart2AnimationDuration animations:^{
            self.containerView.transform = CGAffineTransformScale(CGAffineTransformIdentity, 1.2, 1.2);
        } completion:^(BOOL finished){
            [UIView animateWithDuration:rTransformPart1AnimationDuration delay:0 options:UIViewAnimationOptionCurveEaseOut animations:^{
...
-(void)popViewClose{
    if ([self.delegate respondsToSelector:NSSelectorFromString(@"popViewClose")]) {
        [self.delegate popViewClose];
    }
}

-(void)popViewClick{
    if ([self.delegate respondsToSelector:NSSelectorFromString(@"popViewClick")]) {
        [self.delegate popViewClick];
    }
}
...
-(void)touchesBegan:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event{
    RLog(LD, @"click the interstital image");
    [[AdSdkPopView sharedInstance] close];
    [[AdSdkPopView sharedInstance] popViewClick];
}

//- (void)touchesBegan:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event {
//    if ([AdSdkPopView sharedInstance].tapOutsideToDismiss) {
//        [[AdSdkPopView sharedInstance] hideAnimated:YES withCompletionBlock:nil];
//    }
//}
...
- (void)drawRect:(CGRect)rect{
    ...
    if ([AdSdkPopView sharedInstance].shadeBackgroundType == ShadeBackgroundTypeSolid) {
...
- (instancetype)init {
    ...
    if (self) {
        ...
        [self setBackgroundImage:[UIImage imageNamed:@"AdSdk.bundle/close.png"] forState:UIControlStateNormal];
